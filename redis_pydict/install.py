...
from .ui.debugPrint import *
from .ui.loadingAnim import *
import sys
...
class RedisServer:
  ...
  def install(self, saveLogs: bool = False, ):
    if sys.platform.lower() != 'linux':
      ...
      debug('Support for intalling on other OSs will be implemented in the future.', Log.WRN)
      return
    if saveLogs:
  ...
  async def _startServer(self, threading: bool = False):
    ...
    self.proc.start()
    self.procID = pgrep.pgrep(r"-f 'redis_server\/redis-server \*\:6379'")[0]
